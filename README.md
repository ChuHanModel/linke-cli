...
linke-cli 会把每次命令的**使用行为元数据**上报林课服务器，用于分析用户需求、改进产品：**命令名、CLI 版本、执行结果（exit code）、耗时、身份摘要**（md5(学号+密码)，不可逆）。此遥测**默认开启且不可关闭**（产品决策，与凭据不出本机的原则并存——学号/密码本身永不上传；参数与关键词不入遥测）。上报异步进行（≤2 秒超时静默失败），不影响命令执行。≤1.5.0 的旧版本不含遥测，数据自 1.6.0 起算。成绩回流（给分统计参与）与此无关且有独立开关（`linke config --sync off`）。
